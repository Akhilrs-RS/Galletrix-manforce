...
export default function CRM({ role = "admin" }) {
  ...
  // --- STATEFUL DATA (Loaded from C# .NET API) ---
  ...
  const fetchData = async () => {
    try {
      ...
      // Load contacts with fallback to mock data if C# API is empty
      setContacts(cRes.data && cRes.data.length > 0 ? cRes.data : [
        { id: "c1", name: "John Smith", company: "Tech Corp", email: "john@company.com", phone: "+91 98765 43210", lastContact: "2025-11-10", status: "active" },
        { id: "c2", name: "Sarah Johnson", company: "StartUp Inc", email: "sarah@startup.io", phone: "+91 98765 43211", lastContact: "2025-11-09", status: "lead" },
        { id: "c3", name: "Mike Wilson", company: "Enterprise Ltd", email: "mike@enterprise.com", phone: "+91 98765 43212", lastContact: "2025-11-08", status: "prospect" }
      ]);
      
      // Load activities with fallback to mock data if C# API is empty
      setActivities(aRes.data && aRes.data.length > 0 ? aRes.data : [
        { id: "a1", task: "Follow-up call", contact: "John Smith", date: "2025-11-12", priority: "high", status: "pending" },
        { id: "a2", task: "Product demo", contact: "Sarah Johnson", date: "2025-11-13", priority: "medium", status: "pending" },
        { id: "a3", task: "Proposal sent", contact: "Mike Wilson", date: "2025-11-11", priority: "low", status: "completed" }
      ]);
  ...
  const handleDragEnd = async (event, setter, type) => {
    ...
    if (type === "deals") {
      ...
      if (newStage && activeDeal.stage !== newStage) {
        ...
        try {
          // Send raw stage string to C# controller PUT endpoint in request body
          await api.put(`/crm/deals/${activeId}`, JSON.stringify(newStage), {
  ...
  const handleFormSubmit = async (e, setModalOpen, type = "deals") => {
    e.preventDefault();
    if (type === "deals") {
      try {
        const dealToSubmit = {
  ...
  return (
    <DashboardLayout role={role}>
      <div className="space-y-6">
        ...
        >
                              ...
                              <div className="flex flex-col gap-2 mt-4">
                                ...
                                <select
                                  ...
                                  onChange={async (e) => {
                                    ...
                                    try {
                                      // Send raw stage string to C# controller PUT endpoint in request body
                                      await api.put(`/crm/deals/${deal.id}`, JSON.stringify(newStage), {
                                        headers: { "Content-Type": "application/json" }
                                      });
                                    } catch (err) {
          ...
          {activeTab === "activities" && (
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
              ...
              <div className="p-8 flex flex-row flex-wrap gap-6">
                <SortableContext
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          >
                            {act.task.toLowerCase().includes("call") ? (
                              <Phone size={16} />
                            ) : act.task.toLowerCase().includes("demo") ? (
                              <User size={16} />
                            ) : (
                              <Mail size={16} />
                            )}
        ...
        >
          <form
            onSubmit={(e) => handleFormSubmit(e, setContactModalOpen, "contacts")}
            className="space-y-5"
          >
        ...
        >
          <form
            onSubmit={(e) => handleFormSubmit(e, setActivityModalOpen, "activities")}
            className="space-y-5"
          >
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">
